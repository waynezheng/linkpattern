...
//	console.log(getSpFromUrl(url));
	...
	$(".continue_btn").live("click", function() {
		...
		var $li = $members_ul.children("span");
		var sp = "";
...
//filter members by patterns
function filterMembersByPatterns(ele) {
	if (ele) {
		//for click pattern in the pattern filter
		var label = $(ele).find("span").html();
		var prop_list = $(ele).find(".prop_list").html();
		var intersectionflag = $(ele).attr("intersectionflag");
		//first add tag into the type filter
		var box = $(".filter_box");
...
function getBMCPatternsForFilter() {
//	var uri = "http://dbpedia.org/resource/James_Cameron"; //to do
	var uri = $(".input_uri").html();
	var data = {};
	data["uri"] = uri;
	data["patternfilters"] = getCurrentPatternFilters();
	data = JSON.stringify(data);
	data = ("getBMCLinks3=" + encodeURIComponent(data));
	$.ajax({
		url: linkpattern_servlet,
		async: true,
		data: data,
		...
		success: function(jsob) {
			console.log("getBMCPatternsForFilter successed");
			var results = jsob.bmc;
