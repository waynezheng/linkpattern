...
public class SemanticLinkForSingleEntity implements SemanticLink{
	/**
	 * @param args
	 */
	
	//path index <path,{ends}>
...
//	output
    ...
    public SemanticLinkForSingleEntity(UserData userData, String eUri, HashSet<String> corefCurrentUri, String projectPath){

    	currentUri=eUri;
    	corefCurrentUri.add(currentUri);
    	currentUriSet.addAll(corefCurrentUri);
    	for(String euri:corefCurrentUri){
    		newbuildPathfromCurrentUri(userData, euri, projectPath);
    	}
    ...
    public void newbuildPathfromCurrentUri(UserData userData, String currentUri, String projectPath){	
    	HashMap<String, HashSet<String>> tempallPathMap=findProperty2ValueMap(currentUri, projectPath);
...
//    			sameAsProperties=CommonServiceForLink.getEquivalentPropertyFromDB(forwardpro);
    			...
    			visitedProperties.addAll(sameAsProperties);
...
// <path,{ends}> <end,{paths}>
	...
	public void bordat() {
		boolean topisExisted = false; 
		int totalnum = getAllLinkedEntities().size();
	...
	public ArrayList<String> getEntities(ArrayList<String> props) { 
	...
	public HashMap<String, HashSet<String>> findProperty2ValueMap(String currentUri, String projectPath){
		HashMap<String,HashSet<String>> allPropertyMap= new HashMap<String,HashSet<String>>();
		LogLuceneIndexer.getfilePath(projectPath);
	...
	public ArrayList<Element> getLinkPatternsByBMC08(ArrayList<Element> links,
		...
		ArrayList<Element> newselectedlinkpattern =new ArrayList<Element>();
		if(selectedlinkpattern.size()>10){
			for(int i=0;i<10;i++){
				newselectedlinkpattern.add(selectedlinkpattern.get(i));
			}
		}else{
			for(int i=0;i<selectedlinkpattern.size();i++){
				newselectedlinkpattern.add(selectedlinkpattern.get(i));
			}
		}
		return newselectedlinkpattern;
	}
